...
scores |>
  filter(model %in% models_non_human) |>
  make_performance_table(
    cap = "Performance for two-week-ahead forecasts. Vallues have been cut to three significant digits and rounded.  \\label{tab:scores}"
    ) %>%
  gsub("\\{table\\}", "\\{table\\*\\}", x = .) |>
  cat(file = 'manuscript/performance-table.tex')
...
scores |>
  ...
  make_performance_table(
    hor = 4, 
    cap = "Performance for four-week-ahead forecasts. Vallues have been cut to three significant digits and rounded \\label{tab:scores-4}"
  ) %>%
  gsub("\\{table\\}", "\\{table\\*\\}", x = .) |>
  cat(file = 'manuscript/performance-table-horizon-4.tex')
```
...
plot_ranks <- function(scores, hor = 2) {
  scores |>
    filter(model %in% models_non_human) |>
...
p_ranks <- plot_ranks(scores)
ggsave("output/figures/performance-ranks.png", 
       plot = p_ranks, 
       width = 7, height = 3.4)

p_ranks_4 <- plot_ranks(scores, hor = 4)
