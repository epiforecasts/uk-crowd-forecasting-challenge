...
# Sinificance and bootstrap analysis
...
```{r, performance-table}
make_performance_table <- function(scores, scores_summarised, 
                                   hor = 2) {
  scores |>
    filter(scale %in% c("log", "natural")) |>
...
scores |>
  filter(model %in% models_non_human) |>
  make_performance_table(scores_summarised = scores_summarised) %>%
  gsub("\\{table\\}", "\\{table\\*\\}", x = .) |>
  cat(file = 'manuscript/performance-table.tex')

scores |>
  filter(model %in% models_non_human) |>
  make_performance_table(scores_summarised = scores_summarised, hor = 4) %>%
  gsub("\\{table\\}", "\\{table\\*\\}", x = .) |>
