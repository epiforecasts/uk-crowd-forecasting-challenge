...
# Define Helper Functions, Load Data
...
# Figure SI.1: Data and data revisions
data revisions: https://github.com/CSSEGISandData/COVID-19/issues/6021
...
# Figure SI.4: Weekly participation humans and hub ensemble
...
forecasts_comparison_revised <- filter_comparison_rt_direct(forecasts_revised)
...
scores_forecasts_comparison |> 
  ...
  ggplot(aes(y = interval_score, x = name, group = method, color = method)) + 
  geom_point() + 
  geom_line() + 
...
scores_comparable_ensemble |>
  filter(horizon == 2) |>
  summarise_scores(by = c("target_type", "horizon", "model", "scale"))
...
scores_standardised <- scores |>
  ...
  group_by(model, target_type, scale) |>
  summarise(mean = mean(interval_score)) |>
  pivot_wider(names_from = scale, values_from = mean, names_prefix = "stand_") |>
...
```{r, performance-table}
make_performance_table <- function(scores, standardised_scores) {
  scores |>
    filter(scale %in% c("log", "natural")) |>
    ...
    inner_join(standardised_scores) |>
    mutate(across(c(where(is.numeric)), signif, digits = 2)) |>
    arrange(target_type, model) |>
    mutate(natural = paste0(natural, " (", stand_natural, ")"), 
           log = paste0(log, " (", stand_log, ")")) |>
    select(-stand_log, -stand_natural) |>
...
scores |>
  filter(model %in% models_non_human) |>
  make_performance_table(standardised_scores = scores_standardised) %>%
  gsub("\\{table\\}", "\\{table\\*\\}", x = .) |>
...
## Look at expert forecasts

# ensemble of experts and non-experts
...
# plot_scores_and_pred(
#   forecasts_expert_ensemble,
#   scores_expert_ensemble
# )
...
mean_wis <- scores_df |>
  group_by(scale, expert, target_type, type_and_scale) |>
  summarise(wis_mean = signif(median(interval_score), 3)) |>
  mutate(wis_mean = paste("Mean:", label_fn(wis_mean)))
  
scores_df |>   
  ggplot(aes(x = expert, y = interval_score)) +
  geom_violin(alpha = 0.2) + 
  geom_boxplot(alpha = 0.5) + 
  # scale_fill_brewer(palette = "Set1", name = "Forecast target") + 
  geom_text(data = mean_wis, 
            aes(label = unique(wis_mean), y = -Inf), 
            vjust   = -1) + 
  facet_wrap(~ type_and_scale, scale = "free_y") + 
  scale_y_continuous(labels = label_fn, 
...
scores_df |>
  summarise_scores(by = c("expert", "target_type", "scale", "horizon")) |>
  rename(model = expert) |>
  make_performance_table()
